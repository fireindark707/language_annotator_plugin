(function (global) {
	...
	function resolveMatchSentenceContext(node, matchStart, word, deps) {
		...
		// Build clean container text from filtered text nodes — same space as nodeOffset.
	// Using container.textContent would include extension UI (C2 gloss spans, highlight
	// spans) and contaminate the sentence sent to CleverTranslate.
	const textNodes = collectContainerTextNodes(container, deps);
	let cleanText = "";
	for (let i = 0; i < textNodes.length; i += 1) {
		cleanText += (textNodes[i] && textNodes[i].nodeValue) || "";
	}
	...
	function getContextTextForNode(node, normalizeText) {
		...
		const container = getContextContainerForNode(node) || parentElement;
		// Build text by walking only non-extension nodes, so injected UI text
		// (la-c2-gloss translations, la-word-preview, etc.) is excluded.
		let raw = "";
		const stack = [container];
		while (stack.length) {
			const n = stack.pop();
			if (n.nodeType === Node.TEXT_NODE) { raw += n.nodeValue; continue; }
			if (n.nodeType !== Node.ELEMENT_NODE) continue;
			const cls = n.className;
			if (typeof cls === "string" && cls.split(" ").some(function (c) { return c.startsWith("la-"); })) continue;
			if (n.id === "translationBox") continue;
