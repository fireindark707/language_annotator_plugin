		...
		state.exampleMergeTimer = setTimeout(async () => {
			...
			try {
				...
				Object.keys(batch).forEach((word) => {
					...
					if (WordfreqUtils && WordfreqUtils.isReady(sourceLang) && newOnes.length > 1) {
						newOnes.sort((a, b) =>
							computeContextScore(b.text, word, WordfreqUtils, sourceLang) -
							computeContextScore(a.text, word, WordfreqUtils, sourceLang)
						);
					}
