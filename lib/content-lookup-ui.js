(function (global) {
	...
	function getPreviewSourceLang(WordStorage) {
		...
		return previewSourceLangPromise;
	}
	...
	function translatePreviewSentence(sentence, deps) {
		...
		return getPreviewSourceLang(WordStorage).then((sourceLang) => (
	...
	function translatePreviewWordInContext(word, previewData, deps) {
		...
		return Promise.all([
			...
			typeof detectLanguage === "function" ? Promise.resolve(detectLanguage(sentence)).catch(() => "") : Promise.resolve(""),
			...
			TranslationUtilsRef.requestContextualWordTranslation({
				...
				sourceLang: detectedLang || sourceLang || "auto",
				targetLang,
			})
	...
	function ensureTranslationUiStyles(doc) {
		...
		style.textContent = `
			...
			.la-translation-text.is-contextual {
				font-weight: 700;
				color: #5d3e2f;
				background: rgba(214, 178, 116, 0.14);
				box-shadow: inset 0 -1px 0 rgba(168, 125, 62, 0.14);
			}
		`;
		doc.head.appendChild(style);
	}
