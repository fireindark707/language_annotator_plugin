(function (global) {
	...
	function compactWordsForSync(words, level) {
		...
		for (const [word, dataRaw] of Object.entries(words || {})) {
			...
			if (Array.isArray(data.familyForms)) {
				next.familyForms = data.familyForms
					.filter((x) => typeof x === "string" && x.trim())
					.map((x) => x.trim())
					.slice(0, level >= 2 ? 8 : 20);
			}
			if (data.dictionary && typeof data.dictionary === "object") {
				...
				if (level === 0 && Array.isArray(data.dictionary.entries)) dict.entries = data.dictionary.entries;
	...
	function mergeWordRecord(localData, cloudData) {
		...
		return {
			...
			familyForms: Array.from(new Set([]
				.concat(Array.isArray(local.familyForms) ? local.familyForms : [])
				.concat(Array.isArray(cloud.familyForms) ? cloud.familyForms : [])
				.filter((x) => typeof x === "string" && x.trim())
