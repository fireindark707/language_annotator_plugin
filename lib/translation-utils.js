(function (global) {
	...
	function getEmptyContextualTranslationResult() {
		return {
			translation: "",
			method: "",
			usedContext: false,
	...
	function requestContextualWordTranslation(options) {
		const chromeRuntime = options && options.chromeRuntime;
		const sentence = typeof (options && options.sentence) === "string" ? options.sentence : "";
		const word = typeof (options && options.word) === "string" ? options.word : "";
		const sourceLang = (options && options.sourceLang) || "auto";
		const targetLang = options && options.targetLang;
		const wordPos = options && typeof options.wordPos === "number" && Number.isFinite(options.wordPos)
			? options.wordPos
			: null;
		if (!chromeRuntime || typeof chromeRuntime.sendMessage !== "function" || !sentence.trim() || !word.trim()) {
			return Promise.resolve(getEmptyContextualTranslationResult());
		}
		return new Promise((resolve) => {
			const payload = {
			...
			if (targetLang) payload.targetLang = targetLang;
			if (wordPos !== null && wordPos >= 0) payload.wordPos = wordPos;
			chromeRuntime.sendMessage(payload, (response) => {
				if (chromeRuntime.lastError || !response || typeof response.translation !== "string") {
					resolve(getEmptyContextualTranslationResult());
					return;
				}
				resolve({
					translation: response.translation || "",
					method: typeof response.method === "string" ? response.method : "",
					usedContext: !!response.usedContext,
					fallback: response.fallback !== undefined ? !!response.fallback : !response.translation,
				});
			});
		});
	}
	...
	global.TranslationUtils = {
		...
		requestContextualWordTranslation,
		getEmptyContextualTranslationResult,
