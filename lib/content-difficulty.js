(function (global) {
	...
	function findSentenceContextForOffset(containerText, absoluteOffset, sourceLang) {
		const ExampleUtils = global.ExampleUtils;
		const splitIntoSentences = ExampleUtils && typeof ExampleUtils.splitIntoSentences === "function"
			? ExampleUtils.splitIntoSentences
		...
		const sentences = splitIntoSentences(containerText, sourceLang || "en");
		if (!Array.isArray(sentences) || sentences.length === 0) return null;
		let searchStart = 0;
		for (let i = 0; i < sentences.length; i += 1) {
			const sentence = sentences[i];
			if (!sentence) continue;
			const sentenceStart = containerText.indexOf(sentence, searchStart);
			if (sentenceStart === -1) continue;
			const sentenceEnd = sentenceStart + sentence.length;
			searchStart = sentenceEnd;
			if (absoluteOffset >= sentenceStart && absoluteOffset <= sentenceEnd) {
				return {
					sentence: sentence,
					wordPos: absoluteOffset - sentenceStart,
				};
			}
		}
		return null;
	...
	function buildGlossContext(node, word, wordStart, sourceLang) {
		...
		const sentenceContext = findSentenceContextForOffset(containerText, absoluteOffset, sourceLang);
		if (!sentenceContext || !sentenceContext.sentence) return null;
		return {
			word: word,
	...
	function scheduleC2GlossTranslations(glossContexts, translateFn, translateWordInContext) {
		if (!Array.isArray(glossContexts) || glossContexts.length === 0) return;
		const queue = createTaskQueue(2);
		glossContexts.forEach(function (item) {
			queue(function () {
				const translatePromise = typeof translateWordInContext === "function"
					? translateWordInContext({
						word: item.word,
						sentence: item.sentence,
						wordPos: item.wordPos,
					})
					: (typeof translateFn === "function" ? translateFn(item.word) : Promise.resolve(""));
				return Promise.resolve(translatePromise).then(function (translated) {
	...
	function analyzeAndShow(options) {
		...
		WordfreqUtils.initForLang(sourceLang).then(async function (ready) {
			...
			if ((translateFn || translateWordInContext) && stats.c2Tokens && stats.c2Tokens.size > 0 && !isCharLevelLang(sourceLang)) {
				const c2Words = Array.from(stats.c2Tokens)
					.filter(function (w) { return w.length >= 3; })
					.slice(0, 20);
				const glossContexts = annotateC2InDoc(doc, c2Words, sourceLang);
				setTimeout(function () {
					scheduleC2GlossTranslations(glossContexts, translateFn, translateWordInContext);
				}, 0);
			}
		}).catch(function () {});
	}
