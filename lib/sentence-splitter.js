(function (global) {
	...
	function isExtensionPage() {
		try {
			const p = global.location && global.location.protocol;
	...
	async function init() {
		...
		if (!initPromise) {
			// If the host page started WASM init via a module script and stored
			// the promise on globalThis._sentencexWasmReady (e.g. epub-reader.html),
			// await that instead of a second dynamic import which can fail on
			// extension pages due to MIME-type restrictions.
			if (global._sentencexWasmReady && typeof global._sentencexWasmReady.then === "function") {
				initPromise = global._sentencexWasmReady.then((api) => {
					if (api && typeof api.segment === "function") {
						sentencexApi = api;
						return sentencexApi;
					}
					initFailed = true;
				...
				// Dynamic import only works in extension pages (where the extension's own
				// CSP allows wasm-unsafe-eval). Content scripts run under the host page's
				// CSP, which typically blocks WebAssembly compilation — skip WASM there.
